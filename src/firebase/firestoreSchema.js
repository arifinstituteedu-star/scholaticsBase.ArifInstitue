...
export const saveNoticesToFirestore = (notices, schoolId) => {
    ...
    return saveDocument(
        refs.notices(safeSchoolId),
        withWriteMetadata(payload)
    );
};

export const subscribeToNoticesFromFirestore = (callback, schoolId) => {
    if (!db) return () => {};
    const safeSchoolId = schoolId || 'PROGGA_DEFAULT';
...
export const getUserAccount = async (userId) => {
    ...
    try {
        ...
        const qLower = query(collection(db, COLLECTIONS.users), where('userIdLower', '==', lowerId));
        const qLowerSnap = await getDocs(qLower);
        if (!qLowerSnap.empty) return qLowerSnap.docs[0].data();

        // Fallback scan of all user docs in case of case mismatches
        const allUsersSnap = await getDocs(collection(db, COLLECTIONS.users));
        if (!allUsersSnap.empty) {
            const matchedDoc = allUsersSnap.docs.find((d) => {
                const data = d.data() || {};
                return (
                    d.id.trim().toLowerCase() === lowerId ||
                    String(data.userId || '').trim().toLowerCase() === lowerId
                );
            });
 ...
 * userIdLower query, full collection scan, and cached fallback) to ensure login
 * works seamlessly on new or random devices even if case or cache differs.
 */
export const getUserAccountFresh = async (userId) => {
    ...
    // 4. Try query by userIdLower field
    try {
        const qLower = query(collection(db, COLLECTIONS.users), where('userIdLower', '==', lowerId));
        const querySnap = await getDocs(qLower);
        if (!querySnap.empty) return querySnap.docs[0].data();
    } catch {
        // Fall through
    }

    // 5. Fallback server scan of users collection
    try {
        const allUsersSnap = await getDocs(collection(db, COLLECTIONS.users));
        if (!allUsersSnap.empty) {
            const matchedDoc = allUsersSnap.docs.find((d) => {
    ...
    // 6. Fallback to cached/IndexedDB fetch if server read failed or timed out
...
export const saveUserAccount = (account) => saveDocument(
    ...
    withWriteMetadata({
        ...account,
        userIdLower: String(account.userId || '').trim().toLowerCase(),
