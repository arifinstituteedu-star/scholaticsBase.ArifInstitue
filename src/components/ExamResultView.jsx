...
export default function ExamResultView({ classes = [], defaultToEntry = false, readOnly = false }) {
  ...
  useEffect(() => {
    if (selectedExamSession) {
      ...
      if (selectedExamSession.targetGroup && selectedExamSession.targetGroup !== 'All') {
        setSearchGroup(selectedExamSession.targetGroup);
      } else {
        setSearchGroup('');
      }
      if (showEntryForm && !editingResultKey) {
        setEntryMeta(prev => ({
          ...
          group: selectedExamSession.targetGroup && selectedExamSession.targetGroup !== 'All' ? selectedExamSession.targetGroup : prev.group,
        }));
      }
    } else {
      setSearchClass('');
  ...
  const selectedClassGroups = useMemo(() => {
    ...
    if (selectedClassData?.groupSubjects) {
      Object.keys(selectedClassData.groupSubjects).forEach(g => {
  ...
  const searchClassGroups = useMemo(() => {
    const grpSet = new Set();
    const classObj = safeClasses.find(c => c?.className === searchClass);
    const raw = classObj?.groups || [];
    (Array.isArray(raw) ? raw : []).forEach(g => {
      const name = (typeof g === 'object' && g !== null ? (g.name || g.id || '') : String(g || '')).trim();
      if (name) grpSet.add(name);
    });
...
const getStudentGroupName = (student) => {
  if (!student) return '';
  const grp = typeof student.group === 'object' && student.group !== null
    ? (student.group.name || student.group.id || '')
    : (student.group || student.section || '');
  ...
  // Helper to filter results for an exam card (group-aware)
  const getEnrolledResultsForExamCard = useCallback((examId, targetClass, targetGroup) => {
    const targetClassClean = String(targetClass || '').trim().toLowerCase();
    const targetGroupClean = String(targetGroup || '').trim().toLowerCase();
    const isSpecificGroup = targetGroupClean && targetGroupClean !== 'all' && targetGroupClean !== 'general';
  ...
  }, [allResults, isResultInAllowedClass, classes]);
  ...
  const studentResults = useMemo(() => {
    ...
    const addStudent = (student, className) => {
      ...
      if (!grouped[key]) {
        ...
        grouped[key] = {
          ...
          group: getStudentGroupName(student) || 'N/A',
    ...
    const targetGroupClean = String(selectedExamSession?.targetGroup || '').trim().toLowerCase();
    const isSpecificGroup = targetGroupClean && targetGroupClean !== 'all' && targetGroupClean !== 'general';

    if (Array.isArray(classes) && classes.length > 0) {
      classes
        .filter((cls) => cls && String(cls.className || '').trim().toLowerCase() === targetClassClean)
        .forEach((cls) => {
          (cls.students || [])
            .filter((student) => {
  ...
  const getExpectedSubjects = (student) => {
    ...
    const studentGroup = getStudentGroupName(student) || String(student.group || '').trim();
    let expected = [];
  ...
  const handleReset = () => {
    if (selectedExamSession) {
      setSearchClass(selectedExamSession.targetClass || '');
      setSearchGroup(selectedExamSession.targetGroup && selectedExamSession.targetGroup !== 'All' ? selectedExamSession.targetGroup : '');
    } else {
      setSearchClass('');
      setSearchGroup('');
    }
  ...
  const renderStudentDetails = () => {
    ...
    const getDynamicConfig = (count, nameLength = 0) => {
      const calcSchoolNameFont = (baseSize) => {
        if (nameLength > 32) return Math.min(baseSize, 14.5) + 'px';
        if (nameLength > 24) return Math.min(baseSize, 16.5) + 'px';
        if (nameLength > 18) return Math.min(baseSize, 18.5) + 'px';
        return baseSize + 'px';
      };
      ...
      if (count <= 5) {
        return {
          ...
          schoolNameFont: calcSchoolNameFont(19),
          headerMargin: '12px',
          dividerMargin: '14px',
          studentSectionMargin: '14px',
          infoPadding: '4px 0',
          ...
          tableMargin: '14px',
          tableHeaderPadding: '8px 14px',
          tableHeaderFont: '11.5px',
          tableCellPadding: '8px 14px',
          subjectFont: '13px',
          marksFont: '14.5px',
          highestMarkFont: '13px',
          badgePadding: '4px 12px',
          badgeFont: '12.5px',
          grandTotalPadding: '9px 14px',
          grandTotalFont: '13px',
          summaryMargin: '20px',
          summaryPadding: '9px',
          summaryValueFont: '14.5px',
          footerMarginTop: '24px',
          signatureSpacerHeight: '40px',
        ...
        return {
          ...
          schoolNameFont: calcSchoolNameFont(17),
          headerMargin: '8px',
          dividerMargin: '10px',
          studentSectionMargin: '10px',
          infoPadding: '3px 0',
          ...
          tableMargin: '10px',
          tableHeaderPadding: '6.5px 12px',
          tableHeaderFont: '10.5px',
          tableCellPadding: '6px 12px',
          subjectFont: '12px',
          marksFont: '13px',
          highestMarkFont: '12px',
          badgePadding: '3px 10px',
          badgeFont: '11.5px',
          grandTotalPadding: '7px 12px',
          grandTotalFont: '12px',
          summaryMargin: '16px',
          summaryPadding: '7px',
          summaryValueFont: '13.5px',
        ...
        return {
          ...
          schoolNameFont: calcSchoolNameFont(15),
          headerMargin: '5px',
          dividerMargin: '7px',
          studentSectionMargin: '7px',
          infoPadding: '2px 0',
          infoLabelFont: '9.5px',
          ...
          tableCellPadding: '4px 10px',
          subjectFont: '11px',
          marksFont: '11.5px',
          highestMarkFont: '11px',
          badgePadding: '2px 8px',
          badgeFont: '10.5px',
          grandTotalPadding: '5px 10px',
          grandTotalFont: '11px',
          summaryMargin: '12px',
          summaryPadding: '5px',
          summaryValueFont: '12px',
          footerMarginTop: '14px',
          signatureSpacerHeight: '30px',
        ...
        return {
          ...
          schoolNameFont: calcSchoolNameFont(13.5),
          headerMargin: '3px',
          dividerMargin: '4px',
          studentSectionMargin: '4px',
          infoPadding: '1px 0',
          infoLabelFont: '8.5px',
          infoValueFont: '9.5px',
          photoWidth: '70px',
          ...
          tableMargin: '5px',
          tableHeaderPadding: '3px 8px',
          tableHeaderFont: '9px',
          tableCellPadding: '2.5px 8px',
          subjectFont: '9.5px',
          marksFont: '10.5px',
          highestMarkFont: '10px',
          badgePadding: '1px 6px',
          badgeFont: '9.5px',
          grandTotalPadding: '3.5px 8px',
          grandTotalFont: '10px',
          summaryMargin: '8px',
          summaryPadding: '3px',
          summaryValueFont: '11px',
          footerMarginTop: '10px',
          signatureSpacerHeight: '24px',
    ...
    const activeSchoolNameForCalc = String(getSchoolNameByClass(selectedStudent.class, schoolProfile) || schoolProfile?.schoolName || '').trim();
    const dynamicConfig = getDynamicConfig(subjectCount, activeSchoolNameForCalc.length);
            ...
            >
              ...
              >
                ...
                <div className="transcript-header-section" style={{ textAlign: 'center', marginBottom: dynamicConfig.headerMargin }}>
                  <h1 className="transcript-school-name" style={{ margin: 0, fontSize: dynamicConfig.schoolNameFont, lineHeight: 1.2, fontWeight: '900', color: '#1e3a8a', fontFamily: "'Times New Roman', Georgia, 'Merriweather', serif", textTransform: 'uppercase', letterSpacing: '0.4px', wordBreak: 'break-word', overflowWrap: 'anywhere' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 32 }}>
            {filteredBranchKeys.map((branchKey) => {
              ...
              branchExams.forEach((exam) => {
                const enrolledRes = getEnrolledResultsForExamCard(exam.examId, exam.targetClass, exam.targetGroup);
                  ...
                  {branchExams.length === 0 ? (
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(290px, 1fr))', gap: '18px' }}>
                      {branchExams.map((exam) => {
                        const resultsForExam = getEnrolledResultsForExamCard(exam.examId, exam.targetClass, exam.targetGroup);
                        ...
                        // Determine class enrolled total from classes prop (filtered by targetGroup if specific)
                        const classData = (classes || []).find((c) => c && c.className === exam.targetClass);
                        const examTargetGroupClean = String(exam.targetGroup || '').trim().toLowerCase();
