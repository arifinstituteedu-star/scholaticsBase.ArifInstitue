...
import { getNotices, canUserAccessNotice, subscribeToNoticeUpdates, normalizeRoles } from '../utils/noticeStorage.js';
...
function DetailContent({
      ...
      <div>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
          {[
            ...
            ...(getActiveBranchKeys(schoolProfile).map(k => ({
              key: k,
              label: `${SCHOOL_BRANCHES[k]?.emoji || '🏫'} ${SCHOOL_BRANCHES[k]?.shortName || k}`
            })))
          ].map(b => (
            <button
              key={b.key}
  ...
  if (section === 'teachers_directory') {
    ...
    return (
      <div className="tp-teachers-tab-container" style={{ padding: 0 }}>
        {/* Header Toolbar */}
        ...
        <div className="tp-teachers-search-wrapper">
          ...
          />
          {teacherSearchQuery && (
            <button className="tp-teachers-search-clear" onClick={() => setTeacherSearchQuery('')}>✕</button>
          )}
        </div>
        ...
        <div className="tp-teacher-card-grid">
          {filteredTeachers.length === 0 ? (
            <div className="tp-teachers-empty-state" style={{ gridColumn: '1 / -1' }}>
              <div className="tp-empty-icon">📭</div>
              <h3>No teacher records found</h3>
              <p>Try searching with a different teacher name or subject.</p>
            </div>
          ) : (
            filteredTeachers.map((t, i) => (
              <div key={t.email || t.id || i} className="tp-teacher-card">
                <div className="tp-teacher-card-content">
                  {/* Header Row: Avatar, Name & Subject, Quick Actions */}
                  <div className="tp-teacher-card-header">
                    <div className="tp-teacher-avatar-wrap">
                      {t.profilePic ? (
                        ...
                        <div className="tp-teacher-avatar-circle" style={{ background: 'linear-gradient(135deg, #2563eb 0%, #1d4ed8 100%)' }}>
                    ...
                    <div className="tp-teacher-identity">
                      <h3 className="tp-teacher-name">{t.name || 'Teacher'}</h3>
                      <span className="tp-teacher-subject-badge" style={{ background: '#eff6ff', color: '#1d4ed8', borderColor: '#bfdbfe' }}>
                        {t.subject || 'Faculty Member'}
                      </span>
                    </div>

                    {(t.phone || t.email) && (
                      <div className="tp-teacher-quick-actions">
                        {t.phone && (
                  ...
                  <div className="tp-teacher-contact-list">
                    {t.room && (
                      <div className="tp-contact-chip">
                        <span className="tp-contact-icon">📍</span>
                        <span className="tp-contact-text">Office/Room: {t.room}</span>
                      </div>
                    )}
                    {t.email && (
                      <a href={`mailto:${t.email}`} className="tp-contact-chip">
                        <span className="tp-contact-icon">✉️</span>
                        <span className="tp-contact-text">{t.email}</span>
                      </a>
                    )}
                    {t.phone && (
                      <a href={`tel:${t.phone}`} className="tp-contact-chip">
                        <span className="tp-contact-icon">📞</span>
        ...
        <div className="tp-topbar">
          ...
          <div className="tp-topbar-greeting">
            <div className="tp-topbar-title-row">
              ...
              <h2>{getGreeting()}, <span className="tp-user-name">{displayUser.name || 'Student'}</span>!</h2>
            </div>
            <p className="tp-topbar-subtitle">
              <span>{profile?.schoolName || 'ScholasticBase'}</span>
            ...
            <div className="sv-content-area" style={{ padding: '24px clamp(16px, 3vw, 32px)' }}>
              <div className="tp-notice-toolbar">
                <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
                  ...
                  <span style={{ fontSize: 12, padding: '3px 10px', borderRadius: 12, background: '#f1f5f9', color: '#64748b', fontWeight: 600, border: '1px solid #e2e8f0' }}>
                    🔒 Official Student Bulletin
                  </span>
                </div>
              </div>
              ...
              {notices.length === 0 ? (
                <div style={{ textAlign: 'center', padding: '48px 24px', background: '#f8fafc', borderRadius: '12px', color: '#94a3b8', border: '1px dashed #cbd5e1', marginTop: 16 }}>
                  ...
                  <p style={{ margin: '4px 0 0', fontSize: 13, color: '#94a3b8' }}>Check back later for official school announcements.</p>
                </div>
              ) : (
                <div className="sv-notice-grid">
                  {notices.map((n, i) => (
                    <div key={n.id || i} className={`sv-notice-card ${highlightedNoticeId === n.id ? 'highlight' : ''}`} id={`notice-${n.id || i}`}>
                      ...
                      <div style={{
                        ...
                        <span className="sv-notice-date" style={{ margin: 0 }}>
                          🗓️ {n.date}
                        </span>
                      </div>
