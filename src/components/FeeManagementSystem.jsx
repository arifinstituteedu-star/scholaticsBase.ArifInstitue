...
import { SCHOOL_BRANCHES, getBranchKeyByClass, sortClasses, isSameClass, getActiveBranchKeys } from '../utils/schoolResolver.js';
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="fm-card-top">
                        ...
                        >
                          {branch.shortName && branch.shortName !== branch.name
                            ? branch.shortName
                            : branchKey === 'primary' ? 'Primary Branch' : branchKey === 'secondary' ? 'High School Branch' : 'College Branch'}
                        </span>
                      </div>
          ...
          <form onSubmit={handleRunAutomatedDues} style={{ background: '#f8fafc', borderRadius: 14, padding: 22, border: '1px solid #cbd5e1' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 16, marginBottom: 20 }}>
              ...
              <div>
                ...
                >
                  <option value="ALL">All Active Branches ({getActiveBranchKeys(schoolProfile).length} selected)</option>
                  {getActiveBranchKeys(schoolProfile).includes('primary') && <option value="primary">{SCHOOL_BRANCHES.primary?.name || 'Primary School'}</option>}
                  {getActiveBranchKeys(schoolProfile).includes('secondary') && <option value="secondary">{SCHOOL_BRANCHES.secondary?.name || 'High School'}</option>}
                  {getActiveBranchKeys(schoolProfile).includes('college') && <option value="college">{SCHOOL_BRANCHES.college?.name || 'College'}</option>}
                </select>
              </div>
