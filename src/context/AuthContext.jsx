...
const getLocalUser = (userId) => {
  ...
  // 1. Direct key match in loaded users dictionary
  const matchedKey = Object.keys(users).find((k) => k.toLowerCase() === normalized);
  if (matchedKey && users[matchedKey]) return users[matchedKey];

  // 2. Search values inside users object / array
  const userList = Array.isArray(users) ? users : Object.values(users || {});
  const matchedItem = userList.find((u) => {
    if (!u || typeof u !== 'object') return false;
    return (
      String(u.userId || '').trim().toLowerCase() === normalized ||
      String(u.id || '').trim().toLowerCase() === normalized ||
      String(u.email || '').trim().toLowerCase() === normalized ||
      String(u.name || '').trim().toLowerCase() === normalized ||
      String(u.roll || '').trim().toLowerCase() === normalized ||
      String(u.phone || '').trim().toLowerCase() === normalized
    );
  });
...
export function AuthProvider({ children }) {
    ...
    if (isOnline) {
      try {
        ...
        if (remoteAccount) {
          ...
          const targetKey = remoteAccount.userId || account?.userId || trimmedUserId;
          persistLocalUsers({ ...latestUsers, [targetKey]: account });
        } else {
  ...
  const createUser = async ({ userId, name, password, role, isSuperAdmin = false, classTeacherKey = '', classTeacherClassIdxList = [], classTeacherClassNames = [], classTeacherClassIdx = '', classTeacherClassName = '', allowUpdate = false }) => {
    ...
    const latestUsers = loadLocalUsers();
    let existingUserKey = Object.keys(latestUsers).find(key => key.toLowerCase() === normalizedUserId.toLowerCase());
    let existingData = existingUserKey ? latestUsers[existingUserKey] : null;

    // Check Firestore if not found in local storage cache
    if (!existingData) {
      try {
        const remoteUser = await getUserAccountFresh(normalizedUserId);
        if (remoteUser) {
          existingData = remoteUser;
          existingUserKey = remoteUser.userId || normalizedUserId;
        }
      } catch {
        // Ignore lookup error
      }
    ...
    if (existingData && !allowUpdate) {
      const roleLabel = String(existingData.role || 'user').toUpperCase();
      throw new Error(`Username/ID "${normalizedUserId}" is already taken by an existing ${roleLabel} account. Duplicate usernames are not allowed.`);
    }
    ...
    const newUser = {
      ...
      userIdLower: targetUserKey.toLowerCase(),
      name: normalizedName,
  ...
  const deleteUser = async (userId) => {
    ...
    const targetAccount = matchedKey ? currentUsers[matchedKey] : null;

    const isCallerSuperAdmin = !!(user?.isSuperAdmin || String(user?.userId || '').trim().toLowerCase() === '@@siam##' || String(user?.role || '').toLowerCase() === 'superadmin');
    const isCallerAdmin = isCallerSuperAdmin || String(user?.role || '').toLowerCase() === 'admin';
    if (!isCallerSuperAdmin && targetAccount) {
      const targetRole = String(targetAccount.role || '').toLowerCase();
      if (isCallerAdmin) {
        if (targetRole === 'admin' || targetAccount.isSuperAdmin) {
          throw new Error('Only Super Admin can delete Admin accounts.');
        }
      } else {
        if (targetRole === 'admin' || targetRole === 'principal' || targetAccount.isSuperAdmin) {
          throw new Error('Only Super Admin or Admin can delete Admin or Principal accounts.');
        }
      }
    }
