...
const loadSchoolProfile = () => {
    ...
    try {
        ...
        let parsedActiveBranches = parsed.activeBranches;
        if (!parsedActiveBranches) {
            const rawActiveBranches = window.localStorage.getItem('schoolActiveBranches');
            if (rawActiveBranches) {
                try { parsedActiveBranches = JSON.parse(rawActiveBranches); } catch {}
            }
        }

        const resolvedActiveBranches = {
            primary: parsedActiveBranches?.primary !== false,
            secondary: parsedActiveBranches?.secondary !== false,
            college: parsedActiveBranches?.college !== false,
        };

        if (!resolvedActiveBranches.primary && !resolvedActiveBranches.secondary && !resolvedActiveBranches.college) {
            resolvedActiveBranches.primary = true;
            resolvedActiveBranches.secondary = true;
            resolvedActiveBranches.college = true;
        }
        ...
        const profile = {
            ...
            activeBranches: resolvedActiveBranches,
        };
...
const saveSchoolProfile = (profile) => {
    ...
    try {
        ...
        if (profile?.activeBranches) {
            window.localStorage.setItem('schoolActiveBranches', JSON.stringify(profile.activeBranches));
        }
    } catch {
