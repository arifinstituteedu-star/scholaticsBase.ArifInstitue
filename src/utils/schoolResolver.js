...
/**
 * Reads custom active branch configuration saved in localStorage if available.
 */
export function getCustomActiveBranches() {
  try {
    if (typeof window !== 'undefined') {
      const storedActive = window.localStorage.getItem('schoolActiveBranches');
      if (storedActive) {
      ...
      if (profileRaw) {
        const parsed = JSON.parse(profileRaw);
        if (parsed?.activeBranches) {
          return parsed.activeBranches;
        }
      }
    }
  } catch {
  ...
  return { primary: true, secondary: true, college: true };
}

/**
 * Returns an array of active/enabled branch keys (e.g. ['primary', 'secondary'] or ['secondary']).
 * @param {object} [customSource] - Optional schoolProfile or activeBranches object
 * @returns {Array<'primary'|'secondary'|'college'>}
 */
export function getActiveBranchKeys(customSource) {
  const activeObj =
    customSource?.activeBranches ||
    (customSource && typeof customSource === 'object' && ('primary' in customSource || 'secondary' in customSource || 'college' in customSource) && !customSource.name ? customSource : null) ||
    getCustomActiveBranches() ||
    {};

  const keys = ['primary', 'secondary', 'college'].filter(key => activeObj[key] !== false);
  return keys.length > 0 ? keys : ['primary', 'secondary', 'college'];
}
...
export function getResolvedBranches(customSource) {
  ...
  return {
    primary: {
      ...
      active: activeObj.primary !== false,
    },
    secondary: {
      ...
      active: activeObj.secondary !== false,
    },
    college: {
      ...
      active: activeObj.college !== false,
    },
  };
}
